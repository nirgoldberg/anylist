<?php
/**
 * Widget Name: ANYLIST Widget
 *
 * @author		Nir Goldberg
 * @package		widgets/anylist
 * @version		1.0
 */
...
add_action('widgets_init', function() {
	...
	// register widget
	register_widget( 'ANYLIST_Widget' );

	// enqueue widget frontend style
	anylist_enqueue_skin( anylist_get_setting('active_skin') );

	// enqueue widget frontend style for RTL locale
	if ( is_rtl() ) {
		anylist_enqueue_skin( anylist_get_setting('active_skin') . '-rtl' );
	}

});

class ANYLIST_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$this->widget_id			= 'ANYLIST_Widget';
		$this->widget_name			= __( 'Advanced WooCommerce Products Filter', 'anylist' );
		$this->widget_cssclass		= 'anylist anylist-widget';
		$this->widget_description	= __( 'Add WooCommerce products filter, based on price and product custom taxonomies', 'anylist' );
	...
	function widget($args, $instance) {
		...
		anylist_widget_front()->initialize( $instance['show_categories_menu'], $instance['show_price_filter'], $instance['price_title'], $instance['taxonomies'] );
	...
	function admin_enqueue_scripts() {
		...
		wp_enqueue_style('anylist-admin-style');
	 ...
	 * get_form_field
	 *
	 * This function will return a value from the form_fields array found in ANYLIST_Widget object
	 *
	 * @since		1.0
	...
	private function get_form_field( $name, $default = null ) {
		...
		$form_field = anylist_maybe_get( $this->form_fields, $name, $default );
	...
	private function generate_widget_form() {
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('.<?php echo $this->get_field_id('add-tax'); ?>').click(function() {
					var tax =
						'<div class="tax">' +
							'<div class="inline-tax inline-left"><?php _e( 'Title', 'anylist' ); ?>: <label for="' + taxonomiesField + '[' + count + '][title]"><input class="widefat" id="' + taxonomiesField + '[' + count + '][title]" name="' + taxonomiesField + '[' + count + '][title]" type="text" value="" /></label></div>' +
							'<div class="inline-tax"><?php _e( 'Taxonomy', 'anylist' ); ?>: ' +
								...
								'</label>' +
							'</div>' +
							'<span class="remove-tax"><?php _e( 'Remove', 'anylist' ); ?></span>' +
						'</div>';
		...
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title', 'anylist' ); ?>: <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr( $this->form_fields['title'] ); ?>" /></label></p>
		<p><label for="<?php echo $this->get_field_id('show_categories_menu'); ?>"><input id="<?php echo $this->get_field_id('show_categories_menu'); ?>" name="<?php echo $this->get_field_name('show_categories_menu'); ?>" type="checkbox" <?php echo esc_attr($show_categories_menu) ? 'checked' : ''; ?> /><?php _e( 'Show Product Categories Menu', 'anylist' ); ?></label></p>
		<p><label for="<?php echo $this->get_field_id('show_price_filter'); ?>"><input id="<?php echo $this->get_field_id('show_price_filter'); ?>" name="<?php echo $this->get_field_name('show_price_filter'); ?>" type="checkbox" <?php echo esc_attr($show_price_filter) ? 'checked' : ''; ?> /><?php _e( 'Show Price Filter', 'anylist' ); ?></label></p>
		<p><label for="<?php echo $this->get_field_id('price_title'); ?>"><?php _e( 'Price Filter Title', 'anylist' ); ?>: <input class="widefat" id="<?php echo $this->get_field_id('price_title'); ?>" name="<?php echo $this->get_field_name('price_title'); ?>" type="text" value="<?php echo esc_attr($price_title); ?>" /></label></p>
		<div>
			<label><?php _e( 'Product Taxonomies', 'anylist' ); ?>:</label>
			
			<div class="anylist-taxonomies" id="<?php echo $this->get_field_id('taxonomies'); ?>">
				<?php echo implode('', $taxonomies_html); ?>
			</div>
			
			<span class="anylist-add-tax <?php echo $this->get_field_id('add-tax'); ?>">+ <?php _e( 'Add Taxonomy', 'anylist' ); ?></span>
		</div>
			...
			foreach ($taxonomies as $tax) {
				...
				if ( isset($tax['name']) ) {
					...
					$taxonomies_html[] = sprintf(
						'<div class="tax">' .
							'<div class="inline-tax inline-left">' . __( 'Title', 'anylist' ) . ': <label for="%1$s[%2$s][title]"><input class="widefat" id="%1$s[%2$s][title]" name="%1$s[%2$s][title]" type="text" value="%3$s" /></label></div>' .
							'<div class="inline-tax">' . __( 'Taxonomy', 'anylist' ) . ': ' .
								...
								'</label>' .
							'</div>' .
							'<span class="remove-tax">' . __( 'Remove', 'anylist' ) . '</span>' .
						'</div>',
