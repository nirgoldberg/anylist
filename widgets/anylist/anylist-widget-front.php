...
if ( ! class_exists('ANYLIST_Widget_Front') ) :

class ANYLIST_Widget_Front {
	 ...
	 * __construct
	 *
	 * A dummy constructor to ensure ANYLIST_Widget_Front is only initialized once
	 *
	 * @since		1.0
	 * @param		N/A
	 * @return		N/A
	 ...
	 * initialize
	 *
	 * The real constructor to initialize ANYLIST_Widget_Front
	 *
	 * @since		1.0
	 ...
	 * get_attribute
	 *
	 * This function will return a value from the attributes array found in ANYLIST_Widget_Front object
	 *
	 * @since		1.0
	...
	function get_attribute( $name, $default = null ) {
		...
		$arrtibute = anylist_maybe_get( $this->attributes, $name, $default );
	 ...
	 * set_attribute
	 *
	 * This function will update a value into the attributes array found in ANYLIST_Widget_Front object
	 *
	 * @since		1.0
	...
	private function display_products_filter() {
		...
		<script>
			_ANYLIST_products_filter_wpml_lang				= '<?php echo $wpml_lang; ?>';
			_ANYLIST_products_filter_show_categories_menu	=  <?php echo ( $show_categories_menu ) ? 1 : 0; ?>;
			_ANYLIST_products_filter_show_price_filter		=  <?php echo ( $show_price_filter ) ? 1 : 0; ?>;
			_ANYLIST_products_filter_tax_list				=  <?php echo json_encode( $tax_list ); ?>;
			_ANYLIST_products_filter_min_price				=  <?php echo $min_price; ?>;
			_ANYLIST_products_filter_max_price				=  <?php echo $max_price; ?>;
			_ANYLIST_products_filter_min_handle_price		=  <?php echo $min_handle_price; ?>;
			_ANYLIST_products_filter_max_handle_price		=  <?php echo $max_handle_price; ?>;
			_ANYLIST_products_filter_categories			=  <?php echo json_encode( $categories ); ?>;
			_ANYLIST_products_filter_taxonomies			=  <?php echo json_encode( $taxonomies ); ?>;
			_ANYLIST_products_filter_products				=  <?php echo json_encode( $products ); ?>;
			_ANYLIST_products_filter_currency				= '<?php echo html_entity_decode( get_woocommerce_currency_symbol() ); ?>';
			_ANYLIST_products_filter_ajaxurl				= '<?php echo ( $wpml_lang ) ? str_replace( "/$wpml_lang/", "/", admin_url( "admin-ajax.php" ) ) : admin_url( "admin-ajax.php" ); ?>';		// workaround for WPML bug
			_ANYLIST_products_filter_not_found				= '<?php echo "<p class=\"woocommerce-info\">" . __( "No products were found matching your selection.", "anylist" ) . "</p>"; ?>';
		</script>
		...
		<?php
			...
			wp_enqueue_script( 'anylist-products-filter' );
		?>
		...
		echo '<div class="widgetcontent">';
			...
			/**
			 * anylist_before_filter_content hook
			 */
			do_action( 'anylist_before_filter_content' );

			// categories menu
			if ( $show_categories_menu && $categories ) {
				anylist_get_template_part( 'anylist-widget/anylist-widget', 'categories-menu' );
			}

			// price filter
			if ( $show_price_filter ) {
				anylist_get_template_part( 'anylist-widget/anylist-widget', 'price-filter' );
			}
			
			// taxonomy filters
			anylist_get_template_part( 'anylist-widget/anylist-widget', 'tax-filter' );

			/**
			 * anylist_after_filter_content hook
			 */
			do_action( 'anylist_after_filter_content' );
				...
				$output .= '<ul class="children children-depth-' . $depth . '">';
					...
					$output .=	'<li class="cat-' . $cat_id . '-all">' .
									...
									'<label for="product_cat-' . $cat_id . '-all' . '">' .
										'<span>' . apply_filters( 'anylist_all_subcategories_title', __('All', 'anylist') ) . ' ' .
											'(<span class="count">' . $category[0] . '</span>)' .
										'</span>' .
									'</label>' .
								'</li>';
...
/**
 * anylist_widget_front
 *
 * This function initialize ANYLIST_Widget_Front
 *
 * @since	1.0
 * @param	N/A
 * @return	(object)
 */
function anylist_widget_front() {

	global $anylist_widget_front;

	if( ! isset($anylist_widget_front) ) {

		$anylist_widget_front = new ANYLIST_Widget_Front();

	}

	// return
	return $anylist_widget_front;
